    ...
    for i, item in enumerate(rowsData):
        for column in range(dataSetLen):
                ...
                if countColumn == dataSetLen - 1:
                    ...
                    if countEffector == 1:
                        dataHead.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
                    elif countEffector == 2:
                        dataTorso.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
                    elif countEffector == 3:
                        dataRArm.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
                    elif countEffector == 4:
                        dataLArm.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
                    # elif countEffector == 5:
                    #     dataRLeg.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
                    # elif countEffector == 6:
                    #     dataLLeg.append([dataSet[4], dataSet[6], dataSet[5], dataSet[0], dataSet[2], dataSet[1]])
...
def writeCSVReference(dataSet, filePath = None, referenceFrame = "ROBOT", whichEffectors = "ALL"):
    ...
    elif whichEffectors == "ARMS":
        effectors = ["RARM", "LARM"]
    elif whichEffectors == "LEGS":
        effectors = ["RLEG", "LLEG"]
    else:
        error.abort("is not a valid Effector definition", whichEffectors)        
        ...
        with open(fileDir, 'w') as csvfile:
            ...
            if whichEffectors == "ALL":
                ...
                fieldnames = [
                    ...
                    whichEffectors == "HEAD" or whichEffectors == "TORSO" or whichEffectors == "RARM" or
                    whichEffectors == "LARM" or whichEffectors == "RLEG" or whichEffectors == "LLEG"
            ):
                fieldnames = [
                    'WX', 'WY', 'WZ', 'X', 'Y', 'Z'
                ]
...
def writeCalibrationProfile(coefficients, fileName = None):
    ...
    if fileName is None:
        ...
        fileCounter = 1
        while os.path.exists(temp_path + str(fileCounter)) == False:
            try:
